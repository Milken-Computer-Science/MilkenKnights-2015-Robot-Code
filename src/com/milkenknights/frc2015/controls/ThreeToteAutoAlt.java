 ...
 */
public class ThreeToteAutoAlt extends AutonomousList {
    LinkedList<AutonomousAction> actions;
    
    public ThreeToteAutoAlt(Subsystems subsystems) {
        super(subsystems);
        
        actions = new LinkedList<AutonomousAction>();
        
        // commented this out because we probably wont need it for this alternate version,
        // which should finish in time
        ...
        for (int i = 0; i < 2; i++) {
            ...
            actions.add(new ElevatorMoveAction(subsystems,
                    Constants.ELEVATOR.HEIGHTS.CLEAR_BIN,
                    Constants.ELEVATOR.ACCURACY_THRESHOLD));
            
            // move towards the bin in front of us
            actions.add(new IntakeActuatorsSet(subsystems, ActuatorsState.OPEN));
            actions.add(new PIDStraightAction(subsystems, (i*cycle_dist)+37, 1.75));
            
            actions.add(new AutonWait(0.15));
            
            // rotate to the side, towards the wall, while we are holding the bin
            actions.add(new PIDPivotAction(subsystems, -45, 7.5));
            ...
            actions.add(new PIDPivotAction(subsystems, 0, 4));
